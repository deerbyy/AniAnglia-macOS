...
- **Серии** (НОВОЕ): `EpisodesView` — пикер озвучек (`types`), пикер плееров (`sources`), список серий с бейджами «просмотрено». НОВОЕ v0.15: поиск по сериям/host, фильтр «Все / Не просмотрено / Просмотрено», счётчик `отфильтровано/всего`, счётчик просмотренных и кнопка «Продолжить» на первую непросмотренную серию. НОВОЕ v0.20: sheet плеера получил кнопки «Предыдущая», «Следующая» и «Следующая непросмотренная» в контексте текущего отфильтрованного списка; при переходе/закрытии текущая серия помечается просмотренной для авторизованных. Плеер в sheet через WKWebView; YouTube открывается прямым embed-запросом с Referer, Kodik/Libria — iframe-wrapper’ом, схема-лесс URL `//...` обрабатывается.
- **Скриншоты**: полноэкранный просмотрщик с навигацией ←/→.
- **Закладки**: список по категориям (1–5), из экрана релиза добавляем/убираем.
- **Профиль**: статистика + login form. НОВОЕ v0.18: у чужого профиля плитки пяти списков открывают публичный `ProfileListView` по `/profile/list/all/{profileId}/{cat}/{page}` с сортировкой Anixart, локальным поиском, пагинацией и переходами в релизы. НОВОЕ v0.19: плитка/секция «Друзья» открывает `ProfileFriendsView` по `/profile/friend/all/{profileId}/{page}` с поиском, счётчиком, пагинацией и переходами на профили. НОВОЕ v0.21: соцсети профиля стали кликабельными `Link`-чипами с нормализацией Telegram/VK/Instagram/TikTok/Discord URL; значения без валидной ссылки остаются выделяемым текстом. НОВОЕ v0.22: превью оценок, истории, коллекций, комментариев и видео профиля показывают счётчик `показано/всего` и умеют локально раскрываться/сворачиваться без новых API-запросов.
...
- **Друзья профиля (v0.8/v0.19)**: `GET /profile/friend/all/{profileId}/{page}`, `ProfilesResponse`, `ProfileRoute`, `ProfileFriendsRoute`. В профиле отображается горизонтальный список друзей с аватаром/online/status, кнопкой догрузки страниц и переходом на профиль друга. Нажатие на плитку статистики «Друзья» или кнопку «Все» открывает полный `ProfileFriendsView` с локальным поиском, автодогрузкой и ручной кнопкой «Показать ещё». У чужого профиля счётчики списков открывают публичные списки этого профиля, а не локальные закладки текущего аккаунта.
...
## Что НЕ сделано (TODO)
- [ ] Настоящий auto-next после окончания видео: текущие iframe/WKWebView-плееры не дают надёжного ended-события, поэтому пока есть только ручные кнопки предыдущая/следующая.
