...
**Экраны:**
- **Главная**: «Сейчас смотрят» (`/discover/watching/0`) + «Рекомендации» (если авторизован). НОВОЕ v0.27: домашние rails «Рекомендации», «Сейчас смотрят» и «Коллекции недели» получили ручную догрузку следующих страниц с дедупликацией результатов. НОВОЕ v0.30: «Комментарии недели» на главной получили локальный поиск по тексту/автору/релизу/коллекции, счётчик результатов и раскрытие всего списка вместо жёсткого лимита на 6 комментариев. НОВОЕ v0.31: для авторизованных добавлен rail «Продолжить просмотр» из `/history/0` с переходом во вкладку «История».
- **Каталог** (НОВОЕ): `CatalogView` с фильтрами сортировки/категории/статуса/года. Автодогрузка следующей страницы при прокрутке к последней карточке + ручная fallback-кнопка «Показать ещё». POST `/filter/{page}`. НОВОЕ v0.32: добавлены быстрые пресеты («Онгоинги», «Фильмы», «Популярное 2024+», «Высокая оценка»), chips активных фильтров, быстрый сброс, счётчик релизов и контекстное меню карточек для избранного/пяти списков через общий `ReleaseLibraryContextMenu`.
- **Поиск**: debounced, фокус на TextField автоматически (`@FocusState`), автодогрузка следующей страницы при прокрутке к последней карточке, дедупликация результатов. НОВОЕ v0.32: сохраняет до 10 недавних запросов в UserDefaults, показывает chips недавних запросов с удалением/очисткой, счётчик результатов и контекстное меню карточек для избранного/пяти списков.
- **Релиз**: постер + метаданные + видео-блоки + кадры. НОВОЕ: кнопки «Смотреть» и выпадающее меню закладок (5 категорий + «Убрать»). НОВОЕ v0.16: блок «В коллекциях» умеет менять `CollectionSort`, искать по загруженным коллекциям, показывать счётчик `отфильтровано/всего` и догружать следующие страницы `/collection/all/release/{releaseId}/{page}`. НОВОЕ v0.17: блок «Видео» получил поиск по названию/хостингу/url, фильтр категории и счётчик `отфильтровано/всего`. НОВОЕ v0.24: страница релиза перестроена в логичные секции: герой с постером/метриками/действиями, описание, плитки информации, жанры, кадры, связанные релизы, видео, коллекции и комментарии. НОВОЕ v0.25: жанры перенесены в герой сразу под названием, а связанные релизы фильтруются строже: показываются только совпадения по корню/начальной фразе названия или нескольким сильным токенам с жанровым пересечением; сомнительные результаты поиска скрываются. НОВОЕ v0.26: постер в герое открывается в полноэкранном viewer с zoom/drag, скриншот-viewer тоже получил zoom, а дублирующие мета-чипы под жанрами убраны — год/тип/статус/оценка/голоса живут в верхней сетке метрик. НОВОЕ v0.31: в герое есть блок «В моей библиотеке» со статусом избранного/списка, временем sync и переходом в нужный раздел закладок; меню «Поделиться» копирует название+ссылку, ID или открывает `https://anixart.tv/release/{id}`. Связанные релизы пока строятся через существующий поиск Anixart по названию и сортируются хронологически; когда будет найден точный relation endpoint, можно заменить источник без переделки UI.
...
- **Закладки**: список по категориям (1–5), из экрана релиза добавляем/убираем. НОВОЕ v0.29: на карточках релизов и избранных коллекций во вкладке «Закладки» есть контекстное меню: переместить релиз в другой список, убрать из списка, добавить/убрать избранное, убрать коллекцию из избранных; изменения идут через `BookmarkSyncStore` и синхронизируются с Anixart. НОВОЕ v0.31: сверху добавлен dashboard счётчиков «Избранное», «Коллекции» и всех пяти списков; плитки переключают текущий раздел.
- **Профиль**: статистика + login form. НОВОЕ v0.18: у чужого профиля плитки пяти списков открывают публичный `ProfileListView` по `/profile/list/all/{profileId}/{cat}/{page}` с сортировкой Anixart, локальным поиском, пагинацией и переходами в релизы. НОВОЕ v0.19: плитка/секция «Друзья» открывает `ProfileFriendsView` по `/profile/friend/all/{profileId}/{page}` с поиском, счётчиком, пагинацией и переходами на профили. НОВОЕ v0.21: соцсети профиля стали кликабельными `Link`-чипами с нормализацией Telegram/VK/Instagram/TikTok/Discord URL; значения без валидной ссылки остаются выделяемым текстом. НОВОЕ v0.22: превью оценок, истории, коллекций, комментариев и видео профиля показывают счётчик `показано/всего` и умеют локально раскрываться/сворачиваться без новых API-запросов. НОВОЕ v0.23: общий локальный поиск по активности профиля фильтрует уже загруженные оценки, историю, коллекции, комментарии и видео, включая текст комментариев, источники, авторов, хостинги и метаданные релизов.
- **Настройки** (`Cmd+,`): Основные (НОВОЕ: «Очистить кэш» работает — `RemoteImageCache.shared.clear()`), Воспроизведение, О программе. НОВОЕ v0.31: в «Основных» есть «Данные аккаунта» — полный sync библиотеки, последняя синхронизация и счётчики избранного/коллекций/списков.
...
- **Улучшения коллекций (v0.13/v0.33)**: в `CollectionDetailView` автор коллекции стал ссылкой на `ProfileRoute`; у списка релизов коллекции появился локальный поиск через `Release.matchesLibraryQuery`, счётчик отфильтровано/всего и пустое состояние без сброса пагинации. НОВОЕ v0.33: страница коллекции получила фильтр релизов по состоянию библиотеки (избранное/в списках/без списка/5 категорий), контекстное меню релизов через `ReleaseLibraryContextMenu`, блок покрытия аккаунта, share-меню коллекции с копированием ссылки/ID и открытием в браузере.
- **Поиск и сортировка коллекций (v0.14/v0.33)**: `CollectionsView` получил локальный поиск по загруженным коллекциям через `AnixartCollection.matchesLibraryQuery`, счётчик отфильтровано/всего, пустое состояние с кнопкой догрузки и picker `CollectionSort` для публичного режима (`/collection/all/{page}`). НОВОЕ v0.33: добавлен фильтр по избранным/не избранным коллекциям, синхронизация избранных коллекций при входе, context menu карточек через `CollectionFavoriteContextMenu`, общий сброс фильтров и индикатор sync.
- **История просмотров (v0.4/v0.28/v0.31)**: новый таб в сайдбаре (`SidebarItem.history`), `Features/History/HistoryView.swift`. Требует авторизацию (`/history/{page}`). НОВОЕ v0.28: история получила фильтр по состоянию аккаунта — вся история, избранное, любой список, без списка и все пять категорий закладок; фильтр использует поля релиза и синхронизированный `BookmarkSyncStore`. НОВОЕ v0.31: после поиска/фильтра доступна локальная сортировка «Недавние/Старые/Год/Название», а карточки получили контекстное меню для избранного и пяти списков через `BookmarkSyncStore`.
